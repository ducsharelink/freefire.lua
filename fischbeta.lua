...
function Notify(Des, Time, title)
	Fluent:Notify({
		Title= title or "Fisch Hub Notify",
		Content = Des,
		Duration = Time or 3
	})
end
...
_G['Fisch Hub Table'].SizeUi = (not _G['Fisch Hub Table'].isWindows and UDim2.fromOffset(600,300)) or UDim2.fromOffset(560,600)
if not _G['Fisch Hub Table'].isWindows then
...
ImageButton.Image = "rbxassetid://103731897420638" -- Open image asset ID
...
ImageButton.MouseButton1Click:Connect(function()
    ...
    if isOpen then
        ImageButton.Image = "rbxassetid://103731897420638" -- Replace with close image asset ID
    else
        ImageButton.Image = "rbxassetid://103731897420638" -- Open image asset ID
    end
...
do -- Config Function
	save = function()
		if not isfolder('Fisch Hub') then
			makefolder('Fisch Hub')
		end
...
do -- INFO FISH
	...
	FISHDATA = {
		...
		Pufferfish = {
			...
			Rarity = "Rare";
			Resilience = 65;
			Description = "Pufferfish are clumsy swimmers that can fill their elastic stomachs with huge amounts of water & air to blow themselves up to several times their Fisch size. They do this to evade and scare of predators. On top of their bloating abilities, they also are one of the most poisonous fish in the sea... Also they are the only bony fish which can close their eyes!";
...
AllFuncs['Farm Fish'] = function()
	...
	while Config['Farm Fish'] and task.wait() do
		...
		if LocalPlayer.Character:FindFirstChild(RodName) and LocalPlayer.Character:FindFirstChild(RodName):FindFirstChild("bobber") then
            ...
            XyzClone.Position = XyzClone.Position + UDim2.new(0, 0, 0, -150)
			repeat
				pcall(function()
...
-- Teleporting button for "New Island"
Teleporting:AddButton({
    Title = "New Island",
    Callback = function()
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-2626, 165, 1719)
    end
})

-- Teleporting button for "The Depth"
Teleporting:AddButton({
    Title = "The Depth",
    Callback = function()
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(950, -712, 1267)
    end
})
...
do
	...
	InterfaceManager:SetFolder("Fisch Hub")
